...
<template>
  <div>
    <!-- Header -->
    <div class="mb-6">
      <h2 class="text-lg font-semibold text-gray-900 mb-1">Browse by Category</h2>
      <p class="text-sm text-gray-500">Select a category to filter series</p>
    </div>
    ...
    <!-- Category tree -->
    <div v-else-if="tree.length > 0" class="space-y-3 mb-6">
      <!-- Root categories -->
      ...
      <!-- Subcategories -->
      <div v-if="selectedRoot && selectedRoot.children.length > 0" class="flex gap-2 overflow-x-auto pb-2 pl-4">
        ...
        >
          <span v-if="subcat.icon" class="text-xs">{{ subcat.icon }}</span>
          {{ subcat.name }}
        </button>
      </div>
    </div>
    ...
    <div v-if="activeCategoryId">
      <!-- Loading skeleton -->
      <div v-if="seriesLoading && series.length === 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <div
          v-for="i in 8"
          :key="i"
      ...
      <!-- Series cards -->
      <div v-else-if="series.length > 0" class="space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ...
          >
            <div class="aspect-video bg-gray-100 flex items-center justify-center">
              ...
              <FilmIcon v-else class="h-10 w-10 text-gray-300" />
            </div>
            <div class="p-4">
              <h3 class="text-sm font-semibold text-gray-900 truncate">{{ s.title }}</h3>
              <p v-if="s.description" class="text-xs text-gray-500 mt-1 line-clamp-2">{{ s.description }}</p>
              <div v-if="s.tags && s.tags.length > 0" class="flex flex-wrap gap-1 mt-2">
                <span
                ...
                <span v-if="s.tags.length > 3" class="inline-flex text-xs text-gray-400">
                  +{{ s.tags.length - 3 }}
                </span>
              </div>
            </div>
      ...
      <div v-else class="text-center py-12 bg-gray-50 rounded-lg">
        ...
        <p class="text-gray-500">No series found in this category</p>
      </div>
    </div>

    <!-- No category selected -->
    <div v-else-if="!loading && tree.length > 0" class="text-center py-12 bg-gray-50 rounded-lg">
      <p class="text-gray-500">Select a category to browse series</p>
    </div>
  </div>
</template>
