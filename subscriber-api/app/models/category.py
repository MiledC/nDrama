"""Read-only reference model for the categories table (owned by admin backend)."""
...
# Join table (read-only ref)
category_tags = Table(
    "category_tags",
    Base.metadata,
    Column(
        "category_id", Uuid, ForeignKey("categories.id", ondelete="CASCADE"), primary_key=True
    ),
